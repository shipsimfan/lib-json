    ...
    quote! {
        impl #generics json::Deserialize for #name #generics {
            fn deserialize(value: json::Value, key: Option<&str>) -> Result<Self, json::Error> {
                if !value.is_object() {
                    return Err(json::Error::invalid_type(path, key, json::Type::Object, value));
                }
...
fn map_field(field: Field) -> TokenStream2 {
    ...
    quote! {
        #name : match value.remove(#name_str) {
            Some(value) => json::deserialize(value, path, Some(&format!("{}{}", key_base, #name_str)))?,
            None => return Err(json::Error::missing_field(#name_str, path, key)),
        }
    }
}
    ...
    quote! {
        #name : match value.remove(#name_str) {
            Some(value) => match value.is_null() {
                true => None,
                false => Some(json::deserialize(value, path, Some(&format!("{}{}", key_base, #name_str)))?),
            }
            None => None,
        }
    }
